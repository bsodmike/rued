...
use crate::http::private::{common::Newtype, cstr::CString};
use crate::http::registry::Registry;
use embedded_svc::http::server::{Handler, HandlerError, Request, Response};
...
impl EspHttpServer {
            ...
            match value {
                "application/json" => {
                    if !response_sent {
                        ...
                        if let Err(error2) = Self::render_json_error(raw_req, &error) {
                            warn!(
    ...
    fn render_json_error<E>(raw_req: *mut httpd_req_t, error: E) -> Result<(), EspIOError>
    where
        E: Display,
    {
        let mut headers = Some(EspHttpResponseHeaders::new());
...
impl<'a> Request for EspHttpRequest<'a> {
    type Read<'b> = &'b mut EspHttpRequest<'a>
    where
        'a: 'b;
